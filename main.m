...
%% 参数设置
Capacity=250;%容量，Kwh
Fuel_Consumption=0.95;%燃料消耗速度，kwh/km
...
for i=1:size(line_ori,2)
    ...
    %每条线路多少车，至少满足每20min一班车20min-2*D/V+2*D/V*(x-1)>2D-Ax/Vcharge
    intcon=1;
    B=H-4*line_ori(i).distance/V_vehicle+T_layover/60*sum(x)+ ...
        -(2*line_ori(i).distance*Fuel_Consumption-A(end,:)*x*Charge_kwh)/Charge_speed;
    x=[];
    A=-2*line_ori(i).distance/Charge_speed;
    Aeq=[];
    Beq=[];
    lb=[];
    ub=[];
...
Final_Line=[];
for fund=3.33e8*(1:10)%公交系统营业收入 (7.11e9-6.78e9)
