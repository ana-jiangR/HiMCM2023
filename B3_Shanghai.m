clc;clear;close all;
%% 数据提取
data_ori=readtable('Shanghai.xlsx',"TextType","string","TreatAsMissing",[".","NA"]);
ind_missing=ismissing(data_ori);%缺省数据序号
B=[];
for i=1:size(data_ori,1)
    line_ori(i).lineid=data_ori{i,2};
    line_ori(i).distance=data_ori{i,3};
    A=[false false false ~ind_missing(i,5:end)];
...
%% 参数设置
Capacity=355;%容量，Kwh
Fuel_Consumption=1;%燃料消耗速度，kwh/km
Charge_speed=100;%充电速度kwh/h
T_layover=5;%单次停靠时间，为5minute
C_vehicle= 2500000;%车辆价格CNY
C_Electric=0.15;%电量价格sek/km
C_Diesel=0.55;%燃油价格sek/km
C_infrastructure=1633095;%充电站sek/statuon
SOC=0.3;%最低能量状态
...
for i=1:size(line_ori,2)
    ...
    %不等式约束系数，AX*v*+C-D《C；AX+C-D》soc*C;
    A=[A;-A]*Charge_kwh;%系数
    ...
    %每条线路多少车，至少满足每20min一班车H-2*D/V+H*(x-1)>2D-Ax/Vcharge
    intcon=1;
...
Final_Line=[];
Line_length=[];
for fund=7.79e8*(1:10)%公交系统营业收入
    %% 考虑成本对线路进行规划，目标函数为最大化电动公交线路里程（里程越长，电动车越优）,线路为01矩阵
    f=-data_ori{:,3};%线路里程,标准化，取负
    intcon=1:length(f);
    %车辆价格+充电站价格（有重复）+燃料节省价格
    A=line_num_vehicle*C_vehicle+line_sum_Estaton*C_infrastructure+365*10/H*(C_Electric-C_Diesel)*data_ori{:,3}';
    ...
    Line_length=[Line_length;fval];
end

% writematrix(Final_Line, 'Shanghai_FinalLine.xlsx');
% writematrix(Final_charger_station_Total, 'Shanghai_charger.xlsx');
% writematrix(Final_line_station_Total, 'Shanghai_station.xlsx');
% writematrix(Line_length, 'Shanghai_length.xlsx');


%writematrix(Final_line_vehicle, 'Shanghai_vehicle.xlsx');
data_exist=data_ori(Final_Line(:,10)==1,1:3);
writetable(data_exist,'city.xlsx');
