...
Final_Line=[];
Capacity_scale=160:10:250;
for Capacity=Capacity_scale
...
figure
hold on
plot(Capacity_scale,sum(Final_line_station_Total,2),'-o','LineWidth',2);
plot(Capacity_scale,sum(Final_line_vehicle,2),'-p','LineWidth',2);
xlabel('(Capacity (kwh)')
ylabel('Value')
legend('Line station','Line vehicle')
box on
set(gca,'FontName','Times','FontWeight','bold','FontSize',14)

figure
hold on
plot(Capacity_scale,sum(Final_Line,2),'-s','LineWidth',2);
xlabel('(Capacity (kwh)')
ylabel('Value')
legend('Line num')
box on
