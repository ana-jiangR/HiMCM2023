clc;clear;close all;
%% 数据提取
data_ori=readtable('ChicagoRoutes_New.xlsx',"TextType","string","TreatAsMissing",[".","NA"]);
...
%% 参数设置
Capacity=220;%容量，Kwh
Fuel_Consumption=1.25;%燃料消耗速度，kwh/km
Charge_speed=73;%充电速度kwh/h
T_layover=5;%单次停靠时间，为5minute
C_vehicle=752000;%车辆价格sek
C_Electric=0.19;%电量价格sek/km
C_Diesel=0.27;%燃油价格sek/km
C_infrastructure=251990;%充电站sek/statuon
SOC=0.3;%最低能量状态
...
Final_Line=[];
Line_length=[];
for fund=1.599e8*(1:10)%公交系统营业收入
...
% writematrix(Final_charger_station_Total, 'Chicago_charger.xlsx');
% writematrix(Final_line_station_Total, 'Chicago_station.xlsx');
% writematrix(Line_length, 'Chicago_length.xlsx');
% writematrix(Final_Line, 'Chicago_FinalLine.xlsx');

data_exist=data_ori(Final_Line(:,10)==1,1:2);
writetable(data_exist,'city_CHC10thyear.xlsx');
